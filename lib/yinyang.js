...
function recentFromDrawDate(drawDateStr) {
  // 根據開獎日生成「近期 10 期」模擬數據，使不同日期得到不同的物極必反目標
  var seedBase = parseInt(String(drawDateStr).replace(/-/g, ''), 10) || 20260205;
  var recent = [];
  for (var i = 0; i < 10; i++) {
    var draw = seededDraw(seedBase + i * 11);
    var d = new Date(drawDateStr + 'T12:00:00');
    d.setDate(d.getDate() - (9 - i));
    var dateStr = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
    recent.push({
      draw_num: 'mock/' + String(seedBase + i),
      date: dateStr,
...
function fetchRecentHistory(options) {
  options = options || {};
  var drawDate = options.drawDate; // 'YYYY-MM-DD' 或 undefined

  var recent;
  if (drawDate && /^\d{4}-\d{2}-\d{2}$/.test(String(drawDate).trim())) {
    recent = recentFromDrawDate(String(drawDate).trim());
  } else {
    // 無開獎日時沿用固定近期 10 期
    recent = [
...
function applyYinYangBalance(betGroups, options) {
  ...
  var drawDate = options.drawDate; // 開獎日 'YYYY-MM-DD'，用於近期走勢隨日期變化

  var history = fetchRecentHistory({ drawDate: drawDate });

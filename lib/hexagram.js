...
function computeHexagram(drawDatetime) {
  ...
  // Explicit convention: leap month uses the ordinary month number.
  ...
  return {
    ...
    leap_month_convention: '閏月沿用本月數；年支以農曆年計（不同於八字立春換年）',
    ...deriveHexagrams(upper, lower, changeLine),
...
// Trigram bits are ordered bottom to top, yang=1. Mutual: lines 2–4 / 3–5.
function deriveHexagrams(upper, lower, changeLine) {
  const bits = {1:[1,1,1],2:[1,1,0],3:[1,0,1],4:[1,0,0],5:[0,1,1],6:[0,1,0],7:[0,0,1],8:[0,0,0]};
  const identify = lines => {
    const num = Number(Object.keys(bits).find(k => bits[k].join('') === lines.join('')));
    return {num, ...BAGUA[num]};
  };
  const lines = [...bits[lower], ...bits[upper]];
  const changed = lines.slice();
  changed[changeLine-1] = 1 - changed[changeLine-1];
  return {
    mutual_hexagram:{lower:identify(lines.slice(1,4)),upper:identify(lines.slice(2,5))},
    changed_hexagram:{lower:identify(changed.slice(0,3)),upper:identify(changed.slice(3,6))},
  };
}
